...
from irs_rrt.rrt_params import IrsRrtProjectionParams, DuStarMode
...
q_model_path = os.path.join(models_dir, "q_sys", "magic_finger_6_bimanual_box.yml")
q_model_path_gradients = os.path.join(models_dir, "q_sys", "magic_finger_6_bimanual_box_gradients.yml") # Can use this instead to calculate gradients (reachability_set)
...
# data collection.
data_folder = "ptc_data/box_lift_6"
...
left_finger_t0 = np.zeros(5)
right_finger_t0 = np.zeros(5)
...
class MagicContactSampler(ContactSampler):
    ...
    def pose_to_5_dof(self, pose):
        quat = Quaternion(pose[:4])
        pos = pose[4:]

        d = quat.rotation() @ np.array([0.0, 0.0, 1.0])
        ...
        return np.array([pos[0], pos[1], pos[2], theta, phi])
    ...
    def sample_contact(self, q):
        ...
        for _ in range(1000):
            ...
            q0[idx_q_a_l] = self.pose_to_5_dof(pose_1_global)
            q0[idx_q_a_r] = self.pose_to_5_dof(pose_2_global)
