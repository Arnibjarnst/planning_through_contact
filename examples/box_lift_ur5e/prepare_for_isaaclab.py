...
for i in range(N):
    EE_poses_l[i] = utils.get_ee_pose(joints_l[i], arm_l_pose, "ur5e")
    EE_poses_r[i] = utils.get_ee_pose(joints_r[i], arm_r_pose, "ur5e")
    EE_poses_target_l[i] = utils.get_ee_pose(joints_target_l[i], arm_l_pose, "ur5e")
    EE_poses_target_r[i] = utils.get_ee_pose(joints_target_r[i], arm_r_pose, "ur5e")
...
np.savez_compressed(
    f"RL_data/box_lift_ur5e/IK_{date_str}.npz",
