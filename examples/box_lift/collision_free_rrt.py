...
class CollisionFreeRRT(Rrt):
    def __init__(self, irs_rrt, cf_params, qu, arm_pose):
        ...
        self.qu = np.concatenate([[-100, -100, -100], qu])
    ...
    def sample_subgoal(self):
        ...
        while True:
            qa = ur5_arm.forward(np.random.rand(6) * 2 * np.pi - np.pi)[:3]
            qa += self.arm_pose[4:]
    ...
    def iterate(self):
        ...
        while self.size < self.params.max_size:
            ...
            while collision:
                ...
                dist_from_arm = np.linalg.norm(self.arm_pose[4:] - child_node.q[self.ind_q_a])
...
for n in range(len(segments) - 1):
    ...
    for i, idx_ee in enumerate([idx_q_a_l, idx_q_a_r]):
        arm_pose = arm_l_pose if i == 0 else arm_r_pose
        ...
        cf_rrt = CollisionFreeRRT(prob_rrt, cf_params, qu_start, arm_pose)
        cf_rrt.q_lb = (
